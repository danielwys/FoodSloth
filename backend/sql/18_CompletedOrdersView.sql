...
CREATE VIEW CompletedOrders(month, cname, cid, rider, orderId, restaurantId, restPromoCode, timeRiderDelivered, foodId, price, quantity, foodName, area) AS (
    ...
    SELECT date_part('month', timeRiderDelivered) as month, C.cname, C.cid, U.username, O.orderId, O.restaurantId, O.restPromo, OT.timeRiderDelivered, M.foodId, M.price, OI.quantity, M.foodName, A.area
